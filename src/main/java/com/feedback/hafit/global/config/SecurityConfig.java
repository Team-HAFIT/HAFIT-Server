...
public class SecurityConfig {
    ...
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
        http
                ...
                // == URL별 권한 관리 옵션 ==//
                .authorizeRequests()
                .antMatchers("/api/my/**", "/api/posts/**", "/api/plans/**", "/api/sets/**", "/api/exercises/keyword/**", "/api/exercises", "/api/category", "/api/comments/**").hasAnyRole("USER", "ADMIN")
     ...
     * PasswordEncoder를 사용하는 AuthenticationProvider 지정 (PasswordEncoder는 위에서 등록한
     * PasswordEncoder 사용)
     * FormLogin(기존 스프링 시큐리티 로그인)과 동일하게 DaoAuthenticationProvider 사용
     * UserDetailsService는 커스텀 LoginService로 등록
     * 또한, FormLogin과 동일하게 AuthenticationManager로는 구현체인 ProviderManager 사용(return
     * ProviderManager)
     */
    @Bean
